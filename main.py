...
class Player(pygame.sprite.Sprite):
    ...
    def Bounce_off_wall(self):
        ...
        if not self.face:
            if self.last_ground_pos[0] + 50 > self.rect.midbottom[0] + 6:
                if self.closest_leftRect.left < self.rect.right and self.closest_leftRect.top < self.rect.bottom:
                    self.bounce = False
                    self.rect.right = self.closest_leftRect.left - 3
                else:
                    self.x += 6
                    self.rect.x += 6
        ...
        if self.face:
            if self.last_ground_pos[0] - 50 < self.rect.midbottom[0] - 6:
                if self.closest_rightRect.right > self.rect.left and self.closest_rightRect.top < self.rect.bottom:
                    self.bounce = False 
                    self.rect.left = self.closest_rightRect.right + 3
                else:
                    self.x -= 6
                    self.rect.x -= 6
                    self.y -= round(math.log(6, self.base_values[-1]), 2)
    ...
    def check_collision(self, rects):
        ...
        if self.rect.clipline(self.closest_rightRect.right, self.closest_rightRect.top, self.closest_rightRect.right,
            ...
            if self.rect.bottom == self.y:  # Player on standing
                ...
                self.bounce = True  # Intiates the bouncing off the wall phase
                ...
                self.base_values = [1 + (x * 0.05) for x in
                                    range(int(abs(self.last_ground_pos[0] + 50 - self.closest_rightRect.right) / 6), 0,
                                          -1)]
            else:  # User is falling
                self.move_val = 0 # Cannot Move in the air
                self.rect.x += 6  # Energy Realeased back (Prevents further bugs)
...
class Levels:
    ...
    def Beginner_Room(self):  # Level One
        screen.fill("white")
        wall_left = pygame.Rect(0, 0, 1, 400)
